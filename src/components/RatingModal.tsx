...
export function RatingModal({ tool, lang, onClose }: Props) {
      ...
      <div className="relative w-full max-w-lg bg-white rounded-t-3xl px-6 pt-5 pb-10 shadow-2xl animate-slide-up">
        ...
        {done ? (
          <div className="flex flex-col items-center gap-3 py-6">
            ...
            <p className="text-lg font-semibold text-gray-800">{t.thanks}</p>
          </div>
        ) : (
          <>
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <p className="text-xs text-gray-400 mb-0.5">{t.ratingTitle}</p>
            ...
            <div className="flex justify-center gap-3 mb-6">
              {[1, 2, 3, 4, 5].map((i) => (
                ...
                >
                  <span className={i <= (hovered || stars) ? 'text-amber-400' : 'text-gray-200'}>★</span>
                </button>
              ))}
            </div>
            ...
            <textarea
              ...
              placeholder={t.ratingFeedback}
              rows={3}
              className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-400 resize-none mb-4"
            />
            ...
            <button onClick={() => onClose(false)} className="w-full mt-2 py-2 text-sm text-gray-400">
              {t.skip}
            </button>
          </>
        )}
